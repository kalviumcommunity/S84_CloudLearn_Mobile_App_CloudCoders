...
// Only All and Completed — "In Progress" removed per requirement
enum _CourseFilter { all, completed }
...
class _MyCoursesScreenState extends State<MyCoursesScreen> {
  ...
  // Pull-to-refresh: force re-fetch from Firestore
  ...
  int _watched(CourseInfo c) => _svc.getWatchedCountSync(c.id);
  double _progress(CourseInfo c) {
    final t = c.totalLessons;
    return t == 0 ? 0 : (_watched(c) / t).clamp(0.0, 1.0);
  }
      ...
      body: _loading
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  // ── Summary chips (Total + Completed only) ────────
                  Row(children: [
                    _SummaryChip(
                        label: 'Total',
                  ...
                  // ── Filter chips (All + Completed only) ───────────
                  Row(children: [
                    _FilterChip(
                        label: 'All',
                  ...
                  if (_filter == _CourseFilter.completed &&
  ...
  void _openDetail(CourseInfo course) async {
    ...
    // Cache was already updated inside CourseDetailScreen — just rebuild UI
    if (mounted) setState(() {});
  }
}
...
class _CourseCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isStarted = watchedCount > 0;
